...
function initSplashCursor(config = {}) {
  const canvas = document.getElementById('splashCanvas');
  if (!canvas) {
    console.error('Canvas #splashCanvas no encontrado');
    return;
  }
  ...
  console.log('Canvas dimensionado a:', canvas.width, 'x', canvas.height);
  ...
  if (!glContext) {
    console.error('WebGL no disponible o no se pudo inicializar');
    return;
  }

  const { gl, ext } = glContext;
  console.log('WebGL contexto obtenido correctamente');

  if (!ext.supportLinearFiltering) {
    params.DYE_RESOLUTION = 256;
  ...
  function updateKeywords() {
    let displayKeywords = [];
    if (params.SHADING) displayKeywords.push('SHADING');
  ...
  function updateFrame() {
    ...
    setCanvasSize(); // Asegurar que el canvas tenga el tamaño correcto
  ...
  function updateColors(dt) {
    colorUpdateTimer += dt * params.COLOR_UPDATE_SPEED;
  ...
  function drawDisplay(target) {
    ...
    if (params.SHADING) gl.uniform2f(displayMaterial.uniforms.texelSize, 1.0 / width, 1.0 / height);
    gl.uniform1i(displayMaterial.uniforms.uTexture, dye.read.attach(0));
    blit(target);
  }

  function splatPointer(pointer) {
    let dx = pointer.deltaX * params.SPLAT_FORCE;
    let dy = pointer.deltaY * params.SPLAT_FORCE;
  ...
  function splat(x, y, dx, dy, color) {
    ...
    gl.uniform1f(splatProgram.uniforms.radius, correctRadius(params.SPLAT_RADIUS / 100.0));
    blit(velocity.write);
    velocity.swap();
  ...
  // Exponer splat globalmente para que el intro pueda usarlo
  window.fluidSplat = splat;
...
console.log('splashCursor.js cargado correctamente');

// Esperar a que el DOM esté completamente listo
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    console.log('DOMContentLoaded: Inicializando splashCursor');
    initSplashCursor();
  });
} else {
  console.log('DOM ya listo: Inicializando splashCursor');
  initSplashCursor();
}
